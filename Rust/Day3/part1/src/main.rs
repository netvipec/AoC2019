...
fn intercept(p1a : &(Point, char), p1b : &(Point, char), p2a : &(Point, char), p2b : &(Point, char)) -> Option<(Point, i32)> {
    if (p1b.1 == 'L' || p1b.1 == 'R') &&
       (p2b.1 == 'L' || p2b.1 == 'R') {
        return None;
    }
...
fn main() {
    ...
    let mut point_paths : Vec<Vec<(Point, char)>> = Vec::new();

    for p in paths {
        let mut actual_point = Point::new();
        point_paths.push(vec![(actual_point, ' ')]);
        ...
        for c in p {
            ...
            point_paths[actual_path_index].push((actual_point, cu0));
        }
