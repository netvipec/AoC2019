...
fn valid_password(n : i32) -> bool {
    ...
    for i in 0..digits.len()-1 {
        if (i == 0 && digits[i] == digits[i+1] && digits[i+1] != digits[i+2]) ||
           (i == digits.len() - 2 && digits[i-1] != digits[i] && digits[i] == digits[i+1]) || 
           (0 < i && i < digits.len() - 2 && digits[i-1] != digits[i] && digits[i] == digits[i+1] && digits[i+1] != digits[i+2]) {
            valid = true;
        }
